...
usage ()
{
	echo "Usage: `basename $0` [ -H hostname or IP address ] [ -P port ] [ -u username ] [ -p password ] [ -s session_name ] [ -w warning ] [ -c critical ] [ -V tomcat_version (6 for tomcat pre-7 or 7) ]" >&2
	echo ""
...
NUMSESS=`$WGET -o /dev/null -O - "http://$THE_USERNAME:$THE_PASSWORD@$THE_HOSTNAME:$THE_PORT/$NUMSESS_PATH" | $AWK -F ":" ' /'$SESSION_NAME':/ { SUM+=$3 } END { print SUM }'`
